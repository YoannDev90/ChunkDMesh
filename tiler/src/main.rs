...
use minecraft_map_generator::colors::{BiomePalette, BiomeTintBlocks, BlockPalette};
...
fn parse_args() -> Result<Args, String> {
    ...
    Ok(Args { region_file, chunk_x, chunk_z, _render_all: render_all, output_dir, palette_path, biome_colors_path, biome_tints_path })
}
...
fn main() {
    ...
    if let (Some(cx), Some(cz)) = (args.chunk_x, args.chunk_z) {
        ...
        chunks.par_iter().for_each(|c| {
            let out = render_chunk(c, &config, &block_palette, &biome_palette, &biome_tint_blocks);
            let pn = args.output_dir.join(format!("chunk_{}_{}.png", c.chunk_x, c.chunk_z));
            std::fs::write(&pn, &out.png_data).expect("Failed to write PNG");
        });
